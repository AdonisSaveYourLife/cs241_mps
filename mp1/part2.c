...
int main()
{
    ...
    result = dictionary_add(&dictionary, "key_jizhu", temp);
	if (result != 0) { printf("_add() failed, and it should have been successful.\n"); }
	else { printf("_add(): OKAY!\n"); }
    
    s = dictionary_get(&dictionary, "key_jizhu");
	if (s == NULL || strcmp(s, temp) != 0) { printf("_get() failed or was not the expected result.\n"); }
